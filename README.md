...
![Four stills of 30 agents crossing the random-32-32-20 benchmark map without colliding](docs/screenshots/mapf-plan-filmstrip.png)
30 agents on `random-32-32-20`, the standard small MAPF benchmark map, planned by ECBS at suboptimality 1.1: sum-of-costs 637, 1.024x a proven lower bound, makespan 48. An independent validator re-checks the plan and reports zero conflicts. The same plan as an animation: [GIF](benchmarks/output/swarm-demo.gif), [MP4](benchmarks/output/swarm-demo.mp4).

![87 aircraft morphing from a block into a ring into the word SWARM, in three views](docs/screenshots/formation-morph.png)
A drone-light-show morph: 87 aircraft moving block to ring to text, with goals handed out by the Hungarian assignment. The bottom row is the top-down view the audience never sees, which is where the traffic actually goes.

![Success rate against agent count for seven planners on two benchmark maps](docs/screenshots/success-rate-vs-agents.png)
Where each planner gives up. Success rate within a 20 s budget against agent count on two movingai benchmark maps, measured by `benchmarks/run.py` in one CPU-only Python process.
